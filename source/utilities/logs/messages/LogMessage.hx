package utilities.logs.messages;
...
class LogMessage extends Sprite {
    ...
    public function new(x:Float, y:Float, text:String, color:FlxColor) {
        ...
        var iconID = switch(color) {
            case RED | 0xFF880000:     0;
            case YELLOW:            1;
            case GREEN | 0xFF008800: 2;
            case _:                 3;
        }
        ...
        curColor = color;
        switch(color) {
            case 0xFFFFFFFF | 0xFFFFFF00:
                // make them look darker so that the text is readable
                curColor = FlxColor.multiply(curColor, 0xAAAAAAAA);
            case _:
                // nothing
        }
    ...
    public override function __enterFrame(deltaTime:Int) {
        ...
        // top separator bar
        graphics.beginFill(FlxColor.multiply(curColor, 0x88888888), 1);
